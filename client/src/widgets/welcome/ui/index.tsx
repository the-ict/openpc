    ...
    return (
        <div className='flex flex-col items-center bg-[#050505]'>
            ...
            <section id='welcome' className='text-white flex flex-col items-center mt-20 justify-center px-4 py-20 font-sans select-none'>
                {/* ... (Hero content qismi o'zgarishsiz qoladi) ... */}
                    ...
                    <div className='flex flex-wrap items-center justify-center gap-4 sm:gap-6'>
                        {mounted && token?.length > 0 ? (
                            ...
                            <>
                                ...
                                <Link href="/register" className='text-[#C4D335] button-2 font-medium flex items-center gap-1 hover:underline text-sm md:text-base'>
            ...
            {/* --- 3D BUILD ANIMATION SECTION (Tuzatilgan) --- */}
            <section ref={buildSectionRef} className='relative h-[400vh] w-full'> {/* Balandlikni oshirdik step'lar ko'p bo'lsa smooth bo'lishi uchun */}
                ...
                <div className='sticky top-0 h-screen w-full overflow-hidden'>
                    <Canvas
                        ...
                        dpr={[1, 2]}
                        camera={{ position: [0, 0, 5], fov: 50 }} // Boshlang'ich kamera pozitsiyasi
                    >
...
{/* --- Step Overlay Info (Dinamik va Smooth) --- */}
                    <div
                        className={`pointer-events-none absolute inset-0 flex flex-col justify-end p-6 md:p-12 transition-opacity duration-500 ${
            ...
            {/* TEAM SECTION */}
            {/* ... (Team qismi o'zgarishsiz qoladi) ... */}
            <div className='text-white flex flex-col items-center px-6 md:px-16 py-20 font-sans select-none w-full bg-[#050505]'>
                <section className='w-full max-w-3xl mb-24' id='ourteam'>
                    <h1 className='text-3xl md:text-4xl font-bold mb-4 tracking-tight our-team-text'>Bizning jamoa - siz</h1>
                    ...
                    <div className='flex flex-col gap-3 w-full'>
                        {socialLinks.map((link) => (
                            <div key={link.name} className='w-full cursor-pointer bg-[#D9D9D9] text-black font-bold py-4 px-6 text-sm tracking-wider transition-colors duration-200 hover:bg-gray-300 flex items-center'>
                                <img src={link.icon.src} alt={link.name} className='w-6 h-6 mr-3' />
