        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-12">
            ...
            <div className="bg-[#111] rounded-xl sm:rounded-2xl p-4 sm:p-6">
              ...
              <p className="text-2xl sm:text-3xl font-bold mt-2 text-[#E4E728] font-mono">{formatUSD(Array.isArray(data?.data) ? data?.data.reduce((acc, session) => acc + session.models.reduce((mAcc, model) => mAcc + Number(model.price), 0), 0) / (data.data.length || 1) : 0)}</p>
            </div>
